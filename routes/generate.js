...
router.post('/video', requireShop, async (req, res) => {
  ...
  if (session.videos_used >= limit) {
    return res.status(403).json({ error: 'Video limit reached', limit, plan: session.plan, upgrade: true })
  }
  try {
    ...
    res.json({ success: true, videoId: jobId, downloadUrl: `/generate/download/${filename}`, product: { title: product.title } })
  } catch (err) {
...
router.get('/download/:filename', requireShop, (req, res) => {
  ...
  if (!/^[a-f0-9-]{36}\.mp4$/.test(filename)) return res.status(400).json({ error: 'Invalid filename' })
  const filePath = path.join(__dirname, '../outputs', filename)
  if (!fs.existsSync(filePath)) return res.status(404).json({ error: 'Video not found' })
  res.download(filePath, `viralclip-${Date.now()}.mp4`)
})
...
router.get('/history', requireShop, async (req, res) => {
  ...
  const result = await query(
    `SELECT id, product_title, style, platform, headline, status, created_at
     FROM videos WHERE shop = $1 ORDER BY created_at DESC LIMIT 20`, [shop]
  )
...
router.get('/usage', requireShop, async (req, res) => {
  const { session } = req
  const limit = PLAN_LIMITS[session.plan] || PLAN_LIMITS.free
  res.json({ used: session.videos_used, limit, plan: session.plan, remaining: Math.max(0, limit - session.videos_used) })
})
